...
namespace InterviewPrep.API.Application.Services
{
    public class CategoryService : ICategoryService
    {
        ...
        //Staff - thêm Category
        public async Task<CategoryDTO> AddCategoryAsync(CreateCategoryDTO categoryDto, string createdByUserId)
        {
            var categorytoADD = _mapper.Map<Category>(categoryDto);
            categorytoADD.CreatedBy = createdByUserId;
            var addedCategory = await _categoryRepository.AddCategoryAsync(categorytoADD);
            return _mapper.Map<CategoryDTO>(addedCategory);
        }

        //Staff - sửa thông tin Category
        public async Task<CategoryDTO?> UpdateCategoryInfoAsync(int id, UpdateCategoryInfoDTO updateDto)
        {
        ...
        public async Task<CategoryDTO?> UpdateCategoryStatusAsync(int id, UpdateCategoryStatusDTO updateDto)
        {
            var existingCategory = await _categoryRepository.GetCategoryByIdAsync(id);
            if (existingCategory == null)
            {
